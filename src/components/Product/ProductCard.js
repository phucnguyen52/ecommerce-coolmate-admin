...
const ProductCard = (props) => {
    ...
    const fetchRating = async () => {
        try {
            const req = await fetch(
                `http://localhost:8080/api/products/${value.id}/ratings`
            );
            const res = await req.json();
            if (res.succes) {
    ...
    const onFinish = async (values) => {
        ...
        if (values.NameProducts.trim() !== value.NameProducts) {
            product.NameProducts = values.NameProducts.trim();
        }
        try {
            const req = await fetch(
                `http://localhost:8080/api/products/${value.id}`,
                ...
                <div>
                    <div className="block relative product-img h-[300px] ">
                        <Link to={`/product/${value.id}`}>
                            <img
                                className="rounded-lg relative h-full object-cover w-full"
                                src={JSON.parse(value.Image)[0]}
                            />

                            <img
                                className="rounded-lg hover:opacity-0 absolute top-0 left-0 h-full w-full object-cover"
                                src={JSON.parse(value.Image)[1]}
                            />
                        </Link>
                    </div>
